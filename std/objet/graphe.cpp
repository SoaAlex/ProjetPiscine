...
{
    std::string s0 = "0"; //Départ à 0 arbitraire
    ...
    do{
            ...
            for(const auto ar: m_sommets.find(s.second->getID())->second->getAretes()) {///On regarde ses aretes adjacentes et leur poids
                if( (ar->getCoutFinancier() < coutActuel) && (typeCout == 1)
                    ...
                    sommetActuel = m_sommets.find(s.second->getID())->second;
                        ...
                        && !(marquage.find(ar->getS2()) != marquage.end() && marquage.find(ar->getS1()) != marquage.end()) ) { ///Avec cout environnement. On verifie le type de cout + Si 1 des sommets n'est pas marqué (Le sommet sur lequel on est est forcement marqué mais pas l'autre).
        ...
        listeAretes.insert({AreteActuel->getID(),AreteActuel}); ///L'arrete trouvée peut être ajoutée
    }while((listeAretes.size()-1) != m_sommets.size() );

    Graphe *graphePrim; ///On crée un sous graphe qui correspond à l'algo de Prim
